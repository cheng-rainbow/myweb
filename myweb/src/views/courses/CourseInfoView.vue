  ...
  <section class="relative bg-white-28 px-15vw py-10 pb-24 transition-colors duration-normal dark:bg-black-2A">
    ...
    <div class="rounded-sm bg-white-21 p-5 transition-colors duration-normal dark:bg-black-2B">
      <div v-for="c in courseVideos" :key="c.chapterId">
        <h2 @click="toggleChapterOpen(c)" class="flex cursor-pointer select-none items-center bg-white-28 p-2 text-xl text-black transition-colors duration-normal dark:bg-black-2A dark:text-white">
        ...
        <div :class="!c.isOpenComputed?.value ? 'hidden' : ''">
          <p class="relative h-10 cursor-pointer select-none text-wrap px-3 py-1 leading-8 transition-colors duration-normal hover:bg-white-2A hover:dark:bg-black-2C" v-for="(v, idx) in c.videos" :key="v.videoId">
            ...
            <span>{{ idx + 1 }} -</span>
            {{ v.title }}
            <span class="hidden sm:inline">（{{ v.time }}）</span>
            <span class="absolute right-0 mr-3 px-3"></span>
          </p>
        </div>
      </div>
    </div>
